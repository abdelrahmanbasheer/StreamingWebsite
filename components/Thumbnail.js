...
const Thumbnail = ({result}) => {
  ...
  return (
    <div className="group cursor-pointer p-2 transition  duration-200 ease-in transform sm:hover:scale-105 hover:z-50 ">
        ...
        <p className="capitalize flex items-center opacity-0 group-hover:opacity-100 transition-all duration-100 ease-in-out">{result.media_type&&`${result.media_type} •`}{""}{result.release_date ||result.first_air_date}•{""}<ThumbUpIcon className="h-5 mx-2"></ThumbUpIcon>{result.vote_count}</p>
        </div>
